...
def load_cg_jelly_indexed(
    cg: Dict[str, Any], pkg_dir: pathlib.Path
) -> Optional[Dict[str, Any]]:
    if not isinstance(cg.get("files"), list):
        return None
    if not isinstance(cg.get("functions"), dict):
        return None
    if not isinstance(cg.get("calls"), dict):
        return None
...
def load_cg_generic(
    cg_path: pathlib.Path, pkg_dir: pathlib.Path
) -> Optional[Dict[str, Any]]:
    try:
    ...
    def _get_list(o, k):
        return o.get(k) if isinstance(o.get(k), list) else None

    def _get_dict(o, k):
        return o.get(k) if isinstance(o.get(k), dict) else None

    functions = (
        _get_list(raw, "functions")
        or _get_list(_get_dict(raw, "nodes") or {}, "functions")
        or []
    )
    calls = (
        _get_list(raw, "calls")
        or _get_list(_get_dict(raw, "nodes") or {}, "calls")
        or []
    )
    ...
    for c in calls:
        ...
        sl = (
            c.get("line")
            or (c.get("loc") or {}).get("line")
            or (c.get("position") or {}).get("line")
            or 0
        )
        sc = (
            c.get("column")
            or (c.get("loc") or {}).get("column")
            or (c.get("position") or {}).get("column")
            or 0
        )
        ...
        if file:
            for key in variants(file):
                calls_by_file_line.setdefault(key, {}).setdefault(int(sl), []).append(
                    rec
                )
                calls_by_file_all.setdefault(key, []).append(rec)
...
def resolve_calls_for_node(
    file_s: str, node, src: bytes, cg_idx: Dict[str, Any]
) -> List[Any]:
...
def classify_import_to_dim(pkg: str) -> str:
    ...
    # --- Network / Encoding ---
    if p in NET_CORE or p in NET_LIBS:
        return "D1"
    if p in ENC_CORE or p in ENC_LIBS:
        return "E1"

    # --- Heuristic substrings ---
    if "http" in p or "https" in p or "fetch" in p or "socket" in p:
        return "D1"
    if (
        "crypto" in p
        or "encode" in p
...
def scan_function_or_file(
    ...
    while st:
        ...
        if t == "call_expression":
            ...
            if re.search(r"\bos\.\w+\s*\(", code):
                add_dim(seq, "R2")
            if re.search(r"\bfs\.\w+\s*\(", code):
                add_dim(seq, "R4")
            if (
                re.search(r"\bhttps?\.(request|get|Agent|createServer)\s*\(", code)
                or re.search(r"\b(net|tls|dgram|dns)\.\w+\s*\(", code)
                or re.search(
                    r"\b(axios|fetch|got|request|superagent|needle|undici)\b", code
                )
                or re.search(r"\bnew\s+(WebSocket|WS)\s*\(", code)
            ):
                add_dim(seq, "D2")
            if re.search(
                r"\b(Buffer\.from\(.+?,\s*(['\"])base64\2\)|\.toString\(\s*(['\"])base64\3\)|\batob\s*\(|\bbtoa\s*\(|\bnew\s+TextEncoder\s*\(|\bnew\s+TextDecoder\s*\(|\bcrypto\.(createCipher|createCipheriv|createDecipher|createDecipheriv|createHash|createHmac)\s*\(|\bzlib\.(gzip|gunzip|deflate|inflate|brotliCompress|brotliDecompress)\s*\()",
                code,
                re.I,
            ):
                add_dim(seq, "E2")
            if re.search(
                ...
                code,
            ) or re.search(r"\bchild_process\.(exec|execFile|spawn|fork)\s*\(", code):
                add_dim(seq, "P2")
            if (
                re.search(r"\bprocess\.env(\.|\[)", code)
                or re.search(r"\bos\.(userInfo|homedir|tmpdir)\s*\(", code)
                or re.search(
                    r"(/etc/passwd|id_rsa|\.npmrc|\.ssh/|AppData\\Roaming|%APPDATA%)",
                    code,
                )
            ):
                add_dim(seq, "R5")
            if re.search(
                r"\beval\s*\(|\bnew\s+Function\s*\(|\bFunction\s*\(|\bvm\.(runInNewContext|runInContext|runInThisContext)\s*\(|\bnew\s+vm\.Script\s*\(|\bset(?:Timeout|Interval)\s*\(\s*(['\"])",
                code,
            ):
                add_dim(seq, "P4")
            ...
            m2 = re.search(
                r"require\s*\(\s*(['\"])(?P<mod>[^'\"]+)\1\s*\)\s*\.\s*(?P<meth>[A-Za-z_]\w*)\s*\(",
                code,
            )
            if m2:
                mod = m2.group("mod")
                ...
                if mod in {"process", "child_process"}:
                    ...
                    if re.search(r"\bexec\s*\(", code):
                        if re.search(r"shell\s*:\s*true", code) or shell_re.search(
                            code
                        ):
                            add_dim(seq, "P3")
        ...
        if t in ("string", "template_string", "string_fragment"):
            ...
            if is_url_like(s):
                add_dim(seq, "D3")
            if is_base64_like(s, max(40, long_threshold)):
                add_dim(seq, "E3")
        ...
        if t == "member_expression":
            if re.search(r"\bprocess\.env(\.|\[)", code) or re.search(
                r"\bos\.(userInfo|homedir|tmpdir)\s*\(", code
            ):
                add_dim(seq, "R5")
            if re.search(
                r"\bprocess\.(env|argv|versions|cwd|chdir|exit|kill|pid|ppid|umask|uptime|memoryUsage)\b",
...
def build_sequence_with_cg(
    ...
    for ent in entries:
        ...
        while st:
            ...
            if not jumped:
                fid = (
                    cg_idx["find_fun_by_pos"](
                        canon(str(ent.resolve())), n.start_point[0] + 1
                    )
                    if "find_fun_by_pos" in cg_idx
                    else None
                )
                if fid is not None:
                    for to in cg_idx["fun2fun"].get(fid, []):
...
def build_sequence_entries_only(
    entries: List[pathlib.Path], long_threshold: int, scan_node_modules: bool
) -> List[str]:
    shell_re = compile_shell_re()
    seq: List[str] = []
    for ent in entries:
...
def main():
    ...
    ap.add_argument(
        "--pkg_dir",
        required=True,
        help="Unpacked package root (resolve CG file indices here)",
    )
    ...
    entries = [
        ...
        for l in pathlib.Path(args.entries).read_text(encoding="utf-8").splitlines()
        if l.strip()
    ]
    ...
    if cg_path.exists():
        ...
        if cg_idx and cg_idx.get("call2fun") and cg_idx.get("calls_by_file_all"):
            seq = build_sequence_with_cg(
                pkg_dir, entries, cg_idx, args.long_threshold, scan_nm
            )
